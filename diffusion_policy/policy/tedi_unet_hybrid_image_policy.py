...
class TEDIUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def initialize_buffer(
        ...
        **kwargs,
        ...
        if self.action_buffer is None:
            trajectory = torch.randn(size=shape, dtype=self.dtype, device=self.device, generator=generator)
            ...
            chunk_indices = torch.where(indices >= incomplete_index, num_complete_chunks - 1, chunk_indices)
    ...
    def initialize_buffer_as_zero(self,shape, generator=None):
        ...
        # Add noise
        noise = torch.randn(
            size=action_buffer.shape, dtype=self.dtype, device=self.device, generator=generator
        )
        self.action_buffer = scheduler.add_noise(
                action_buffer, noise, diff_steps
            )
