...
class RealToolImageDataset(BaseImageDataset):
    def __init__(self,
            shape_meta: dict,
            dataset_path: str,
            horizon=1,
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == 'rgb':
            ...
            if 'pose' in key:
                assert tuple(shape) in [(2,),(3,),(6,)] #Give access to all rot channels if one is given. This is because get_obs does not have functionality for zrot only
    
    action_shape = tuple(shape_meta['action']['shape'])
    assert action_shape in [(4,),(5,),(7,)] #3,4,6 + actuation
    ...
    if action_shape == (4,):
        ...
        zarr_resize_index_last_dim(zarr_arr, idxs=[0,1,2,6]) #t2,ake x,y,z,actuator

    if action_shape == (5,):
        zarr_arr = replay_buffer['action']
        zarr_resize_index_last_dim(zarr_arr, idxs=[0,1,2,5,6]) #take x,y,z,zrot,actuator
    
    # else: don't reshape

    for key, shape in lowdim_shapes.items():
        if 'pose' in key and shape == (3,):
