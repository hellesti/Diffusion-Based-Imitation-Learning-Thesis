...
def bidirectional_sampler(strong, weak, obs_dict, prior, num_sample=10, beta=0.99, num_mode=3):
    ...
    # Dict:
    #   image: (1,OH, 3, 200, 200)
    #   lowdim: (1, OH, 6)
    # We want:
    # Dict_batch:
    #   image: (num_sample, OH, 3, 200, 200)
    # pre-process
    B = 1
    obs_dict_batch = dict()
    for key in obs_dict.keys():
        ...
        B = obs_shape[0]
        OH = obs_shape[1]
        obs_shape_len = [1]*(len(obs_shape)-2)
        obs_dict_batch[key] = obs_dict[key].unsqueeze(1).repeat(1, num_sample, 1, *obs_shape_len).view(B * num_sample, OH, *obs_shape[2:])
