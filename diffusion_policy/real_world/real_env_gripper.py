...
class RealEnv(BaseRealEnv):
    ...
    def __init__(self, 
        ...
        self.startpos = [0.59938478, -0.1640019, 0.34, -2.21202159, 2.22647071, 0.00622352]
    ...
    def exec_perturbed_actions(self, 
            actions, 
            timestamps,
            disturbance_vel,
            disturbance_pos,
            ...
            disturbance_func,
            stages: Optional[np.ndarray]=None,
            action_transform: Optional[st.Rotation]=st.Rotation.from_matrix(np.identity(3)),
            ): #This parameter transforms actions for execution but not for recording. Can be chosen conveniently to reduce action dimentionality when under-actuated. (rot. in ee frame)
        assert self.is_ready
        if not isinstance(actions, np.ndarray):
            actions = np.array(actions)
        if not isinstance(timestamps, np.ndarray):
        ...
        for i in range(len(new_actions)):
            ...
            # Add disturbance to action
            disturbance_pos, disturbance_vel,_ = disturbance_func(disturbance_pos, disturbance_vel, disturbance_rng)
            this_action[:2] += disturbance_pos
            gripper_action = this_action[-1]
        ...
        return disturbance_pos, disturbance_vel

    def exec_perturbed_actions_precomputed(self, 
            all_disturbances,
            actions, 
            timestamps,
            stages: Optional[np.ndarray]=None,
        ...
        for i in range(len(new_actions)):
            ...
            # Add disturbance to action
            this_action[:2] += all_disturbances.pop(0)
            gripper_action = this_action[-1]
    ...
    def goto_startpos(self, vel=1.05, accel=1.4):
            #print("Start pos used: ", self.startpos)
            assert self.robot.moveL(self.startpos, vel=vel, accel=accel)
            return True