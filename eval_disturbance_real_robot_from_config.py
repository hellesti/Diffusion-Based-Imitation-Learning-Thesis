...
def main(config, input, output, robot_ip, tool_address, ft_sensor_ip, tool_init_pos, match_dataset, match_episode, vis_camera_idx, steps_per_inference, max_duration, frequency, command_latency, disturbance_seed):
    ...
    scale_factors = np.array([0.005,0.005])*0.5
            ...
            while True:
                ...
                while True:
                    ...
                    if command == "stop":
                        # Exit program
                        env.end_episode()
                        all_disturbance_values = np.array(all_disturbance_values)
                        all_actions = np.array(all_actions)
                        adv_fname = f"logged_values/{cfg.name}_dist_values_ds_{disturbance_seed}.txt"
                        aa_fname = f"logged_values/{cfg.name}_act_values_ds_{disturbance_seed}.txt"
                        np.savetxt(adv_fname, all_disturbance_values)
                        np.savetxt(aa_fname, all_actions)
                        exit(0)
                ...
                try:
                    ...
                    while True:
                        ...
                        if disturbance_active:
                            ...
                            all_disturbance_values.append(disturbance_pos)
